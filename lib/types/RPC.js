...
RPC.prototype._parse = function(type, stream) {
  ...
  stream.on('end', function() {
    var obj = JSON.parse(buf), id, argslen = 0, args, fn, typeinfo;
    if (type === TYPE_RESP || type === TYPE_RESP_NOSER) {
      ...
      if (fn = self.methods[obj[1]]) {
        ...
        if (argslen === 0)
          fn.call(self.this, cb);
        else if (argslen === 1)
          fn.call(self.this, args[0], cb);
        else if (argslen === 2)
          fn.call(self.this, args[0], args[1], cb);
        else if (argslen === 3)
          fn.call(self.this, args[0], args[1], args[2], cb);
        else {
          args.push(cb);
