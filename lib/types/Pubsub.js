...
Pubsub.prototype._findLF = function(chunk, i, len) {
  ...
  for (; i < len; ++i) {
    if (chunk[i] === 0x0A) {
      if (i > start)
        this.buffer += chunk.toString('utf8', start, i);
  ...
  if (start === 0)
    ...
    this.buffer += chunk.toString('utf8', start);
  return len;
};
...
Pubsub.prototype._parse = function(obj) {
  ...
  if (argslen === 0) {
    ...
    args.unshift(ev);
    this.events._emit.apply(this.events.emit, args);
    if (this.events._broadcast) {
      args.unshift('*');
      this.emit.apply(this.events.emit, args);
    }
  }
};
